...
public class TaskExecutorTest extends TestLogger {
	...
	public void testImmediatelyRegistersIfLeaderIsKnown() throws Exception {
		...
		when(rmGateway.registerTaskExecutor(
					anyString(), any(ResourceID.class), anyInt(), anyInt(), any(HardwareDescription.class), any(Time.class)))
		...
		try {
			...
			verify(rmGateway, Mockito.timeout(timeout.toMilliseconds())).registerTaskExecutor(
					eq(taskManagerAddress), eq(taskManagerLocation.getResourceID()), anyInt(), anyInt(), any(HardwareDescription.class), any(Time.class));
		}
		finally {
	...
	public void testInitialSlotReportFailure() throws Exception {
		...
		try {
			...
			testingResourceManagerGateway.setRegisterTaskExecutorFunction(new Function<Tuple5<String, ResourceID, Integer, Integer, HardwareDescription>, CompletableFuture<RegistrationResponse>>() {
				@Override
				public CompletableFuture<RegistrationResponse> apply(Tuple5<String, ResourceID, Integer, Integer, HardwareDescription> stringResourceIDIntegerHardwareDescriptionTuple5) {
